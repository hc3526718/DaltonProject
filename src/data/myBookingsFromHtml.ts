/**
 * My Bookings from `12-Event Booking - My Bookings.html`.
 */
export type UpcomingBookingRow = {
  id: string;
  uri: string;
  dateLine: string;
  title: string;
  time: string;
  place: string;
  status: 'booked' | 'waitlisted';
  scheduleLine: string;
};

export type PastBookingRow = {
  id: string;
  uri: string;
  dateLine: string;
  title: string;
  time: string;
  place: string;
  status: 'completed' | 'cancelled';
};

export const MY_BOOKINGS_UPCOMING_FROM_HTML: UpcomingBookingRow[] = [
  {
    id: 'strength',
    uri: 'https://images.unsplash.com/photo-1517836357463-d25dfeac3438?q=80&w=600&auto=format&fit=crop',
    dateLine: 'OCT 15, 2024',
    title: 'STRENGTH CAMP',
    time: '10:00 AM',
    place: 'Elite Fitness',
    status: 'booked',
    scheduleLine: 'October 15, 2024 • 10:00 AM - 12:00 PM',
  },
  {
    id: 'yoga',
    uri: 'https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?q=80&w=600&auto=format&fit=crop',
    dateLine: 'OCT 22, 2024',
    title: 'YOGA FLOW',
    time: '7:00 AM',
    place: 'Zen Studio',
    status: 'waitlisted',
    scheduleLine: 'October 22, 2024 • 7:00 AM - 8:30 AM',
  },
  {
    id: 'hiit',
    uri: 'https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?q=80&w=600&auto=format&fit=crop',
    dateLine: 'NOV 05, 2024',
    title: 'HIIT BOOTCAMP',
    time: '6:00 PM',
    place: 'Power Gym',
    status: 'booked',
    scheduleLine: 'November 5, 2024 • 6:00 PM - 7:00 PM',
  },
];

export const MY_BOOKINGS_PAST_FROM_HTML: PastBookingRow[] = [
  {
    id: 'cardio',
    uri: 'https://images.unsplash.com/photo-1506629905645-b178a2b92e15?q=80&w=600&auto=format&fit=crop',
    dateLine: 'SEP 20, 2024',
    title: 'CARDIO BLAST',
    time: '9:00 AM',
    place: 'Fitness Hub',
    status: 'completed',
  },
  {
    id: 'pilates',
    uri: 'https://images.unsplash.com/photo-1518611012118-696072aa579a?q=80&w=600&auto=format&fit=crop',
    dateLine: 'SEP 15, 2024',
    title: 'PILATES CLASS',
    time: '11:00 AM',
    place: 'Wellness Center',
    status: 'cancelled',
  },
];
